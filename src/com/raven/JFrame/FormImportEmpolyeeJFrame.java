...
import com.qlcnnh.DAO.EmpolyeeDao;
...
//Phương thức khởi tạo có tham số
    public FormImportEmpolyeeJFrame(String fullname, String role, String gender, String birth, String address, String phone, String email, String salary, int idUser, int status) {
        ...
        setLocationRelativeTo(null); // Đặt vị trí form ở giữa màn hình
        setResizable(false); // Không cho phép thay đổi kích thước form
        setDefaultCloseOperation(DISPOSE_ON_CLOSE); // Đóng form khi nhấn nút close
        btnAddEmployee.setEnabled(false); // Vô hiệu hóa nút Add Employee
        btnAddEmployee.setVisible(false); // Ẩn nút Add Employee

        // Đặt các giá trị truyền vào lên form
    ...
    public void update() {
        ...
        daoE.update(e);// Cập nhật thông tin người dùng trong cơ sở dữ liệu
        MsgBox.alert(this, "Cập nhật thành công !");// Hiển thị thông báo cập nhật thành công
    ...
    EmpolyeeDao emDao = new EmpolyeeDao();// Tạo đối tượng để thao tác với bảng Employee trong cơ sở dữ liệu
    ...
    public void insert() {
        try {//Kiểm tra các điều kiện thông tin
            if (labelValidate.checkEmpty(lblName, txtName, "Tên chưa nhập!") == false) {
                ...
                User e = getFormE();// Lấy thông tin nhân viên từ form
                daoE.insert(e);// Thêm nhân viên vào cơ sở dữ liệu
                Account a = getFormA();// Lấy thông tin tài khoản từ form
                daoA.insert(a); // Thêm tài khoản vào cơ sở dữ liệu
                new MainForm().showForm(new FormListEmpolyee());// Hiển thị form danh sách nhân viên
    ...
    public User getFormE() {
        User e = new User();// Khởi tạo một đối tượng User mới
        e.setFullname(txtName.getText());  // Lấy và thiết lập tên đầy đủ từ trường txtName
        e.setRole(rdoManage.isSelected());
        e.setGender(rdoMale.isSelected());
        e.setDateOfBirth(XDate.toDate(txtBirth.getText(), "dd-MM-yyyy"));
        e.setAdress(txtAdress.getText());
        ...
        e.setStatus(rdoWorking.isSelected());
        e.setSalary(Double.parseDouble(txtSalary.getText()));
// Chuyển đổi chuỗi lương từ trường txtSalary sang kiểu Double và thiết lập cho người dùng
        return e;
    }
    public Account getFormA() {
        Account a = new Account();//Khởi tạo account mới
    ...
    private void initComponents() {
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addComponent(txtBirth, javax.swing.GroupLayout.PREFERRED_SIZE, 300, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
            .addGroup(jPanel2Layout.createSequentialGroup()
                .addGap(61, 61, 61)
